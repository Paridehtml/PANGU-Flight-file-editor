 ...
 *
 * Copyright:
 *   (c) Space Technology Centre, University of Dundee, 2001-2023.
 *
 * History:
 *   22-Oct-2002 (mnd):
 *      Original version.
...
enum
{
	...
	/* Version 1.21 */
	MSG_GET_THERMAL_SOURCES			=  36,
	MSG_GET_THERMAL_SOURCE			=  37,
	MSG_GET_THERMAL_AREA_FACTORS		=  38,
	MSG_GET_EMISSION_SCALE			=  39,
	MSG_GET_IRRADIANCE			=  40,
	MSG_GET_WAVELENGTH			=  41,
	MSG_GET_BANDWIDTH			=  42,
	/* Version 1.22 */
	MSG_GET_DETECTOR_EXPOSURE		=  43,
	MSG_GET_DETECTOR_TEMPERATURE		=  44,
	MSG_GET_DETECTOR_GAIN			=  45,
	MSG_GET_DETECTOR_BIAS			=  46,
	MSG_GET_DETECTOR_QE			=  47,
	MSG_GET_DETECTOR_WELL_CAPACITY		=  48,
	MSG_GET_DETECTOR_READOUT_RMS		=  49,
	...
	/* Version 1.21 */
	MSG_SET_THERMAL_SOURCE			= 300,
	MSG_SET_THERMAL_AREA_FACTORS		= 301,
	MSG_SET_EMISSION_SCALE			= 302,
	MSG_SET_IRRADIANCE			= 303,
	MSG_SET_WAVELENGTH			= 304,
	MSG_SET_BANDWIDTH			= 305,
	MSG_SET_CAMERA_BAND			= 306,
	/* Version 1.22 */
	MSG_SET_DETECTOR_EXPOSURE		= 307,
	MSG_SET_DETECTOR_TEMPERATURE		= 308,
	MSG_SET_DETECTOR_GAIN			= 309,
	MSG_SET_DETECTOR_BIAS			= 310,
	MSG_SET_DETECTOR_QE			= 311,
	MSG_SET_DETECTOR_WELL_CAPACITY		= 312,
	MSG_SET_DETECTOR_READOUT_RMS		= 313,
...
extern joint_data* pan_protocol_get_joints(SOCKET, unsigned long, unsigned long*);
	/*
	 * pan_protocol_get_joints(i, n) gets an array of joint_data with
	 * one entry for each joint of object i. The location pointed to
	 * by n is filled with the number of entries in the joint list. 
	 *      id:   handle used to identify the joint
	 ...
	 * returned and n = 0
	 *
	 * To free the joint list, first call free() on each 'name' field
	 * then call free() on the list itself.
	 *
...
extern frame_data* pan_protocol_get_frames(SOCKET, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_frames(o,n) returns an array of frame_data, with
	 * one entry for each frame of object o. the variable pointed to by n
	 * is set to the number of frames.
	 *
	 * Each entry has the following fields:
	 *      id: handle used to identify the frame
	 *    name: descriptive label for the frame
	 *
	 * If an invalid object is specified, an empty list is
	 * returned and n = 0
	 *
	 * To free the frame list, first call free() on each 'name' field
	 * then call free() on the list itself.
	 *
...
extern thermal_source_data *pan_protocol_get_thermal_sources(SOCKET, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_thermal_sources(s,o,&n) returns an array of
	 * thermal_source_data, with one entry for each source of object o.
	 * The variable pointed to by n is set to the number of sources.
	 *
	 * Each entry has the following fields:
	 *       id: handle used to identify the source
	 *     name: descriptive label for the source
	 *
	 * If an invalid object is specified, an empty list is
	 * returned and n = 0
	 *
	 * To free the source list, first call free() on each 'name' field
	 * then call free() on the list itself.
	 *
	 * IMPLEMENTS GetThermalSources (36)
	 */

extern double *pan_protocol_get_thermal_source(SOCKET, unsigned long, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_thermal_source(s,o,h,&n) returns an array of
	 * values for thermal source h in object o; n is set to the number
	 ...
	 *    2: merge_factor
	 *
	 * If an invalid object is specified, NULL is returned and n is
	 * set to 0.
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetThermalSource (37)
	 */

extern double *pan_protocol_get_thermal_area_factors(SOCKET, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_thermal_area_factors(s,o,&n) returns an array of
	 * area factors for object o; n is set to the number of values in
	 * the array.
	 *
	 * Each entry has the following meaning:
	 *    0: ambient/background area factor
	 *    1: direct solar area factor
	 *    2: indirect solar/reflected albedo area factor
	 *    3: planetary emission area factor
	 *    4: internal heat factor
	 *    5: plate/appendage area factor
	 *
	 * If an invalid object is specified, NULL is returned and n is
	 * set to 0.
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetThermalAreaFactors (38)
	 */

extern double *pan_protocol_get_emission_scale(SOCKET, unsigned long *);
	/*
	 * pan_protocol_get_emission_scale(s,&n) returns the emission scale
	 * as an array; n is set to the number of values in the array
	 * (normally 3).
	 *
	 * Each entry has the following meaning:
	 *    0: R emission scale
	 ...
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetEmissionScale (39)
	 */

extern double *pan_protocol_get_irradiance(SOCKET, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_irradiance(s,b,&n) returns the irradiance in
	 * band b (0=VIS, 1=TIR).
	 *
	 * c = pan_net_get_irradiance_RX(s,&n) returns a pointer to the array
	 * of irradiance components and sets n to the length. If any element
	 * was invalid (non-existant band) then NULL is returned and n is 0.
	 *
	 * Each entry has the following meaning:
	 *    0: irradiance in the R channel [photons/s/m^2]
	 *    1: irradiance in the G channel [photons/s/m^2]
	 *    2: irradiance in the B channel [photons/s/m^2]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetIrradiance (40)
	 */

extern double *pan_protocol_get_wavelength(SOCKET, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_wavelength(s,b,&n) returns the wavelength in
	 * band b (0=VIS, 1=TIR).
	 *
	 * c = pan_net_get_wavelength_RX(s,&n) returns a pointer to the array
	 * of wavelength components and sets n to the length. If any element
	 * was invalid (non-existant band) then NULL is returned and n is 0.
	 *
	 * Each entry has the following meaning:
	 *    0: wavelength in the R channel [m]
	 *    1: wavelength in the G channel [m]
	 *    2: wavelength in the B channel [m]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetWavelength (41)
	 */

extern double *pan_protocol_get_bandwidth(SOCKET, unsigned long, unsigned long *);
	/*
	 * pan_protocol_get_bandwidth(s,b,&n) returns the bandwidth in
	 * band b (0=VIS, 1=TIR).
	 *
	 * c = pan_net_get_bandwidth_RX(s,&n) returns a pointer to the array
	 * of bandwidth components and sets n to the length. If any element
	 * was invalid (non-existant band) then NULL is returned and n is 0.
	 *
	 * Each entry has the following meaning:
	 *    0: bandwidth in the R channel [m]
	 *    1: bandwidth in the G channel [m]
	 *    2: bandwidth in the B channel [m]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetBandwidth (42)
	 */

extern double pan_protocol_get_detector_exposure(SOCKET s, unsigned long c, char *perr);
	/*
	 * pan_protocol_get_detector_exposure(s,c,perr) returns the exposure
	 * time of the detector model of camera c. If perr is non-null then
	 * *perr will be set to 1 if the result is valid or v=0 if not.
	 *
	 * IMPLEMENTS GetDetectorExposure (43)
	 */

extern double pan_protocol_get_detector_temperature(SOCKET s, unsigned long c, char *perr);
	/*
	 * pan_protocol_get_detector_temperature(s,c,perr) returns the
	 * temperature of the detector model of camera c. If perr is non-null
	 * then *perr will be set to 1 if the result is valid or v=0 if not.
	 *
	 * IMPLEMENTS GetDetectorTemperature (43)
	 */

extern double *pan_protocol_get_detector_gain(SOCKET s, unsigned long c, unsigned long *pn);
	/*
	 * pan_protocol_get_detector_gain(s,c,&n) returns the RGB gain of
	 * camera c as an array setting n to the length.
	 * If any element was invalid (e.g. the camera does not exist or does
	 * not have a detector model) then NULL is returned and n is set to 0.
	 *
	 * The returned array elements are:
	 *    0: gain in the R channel [e-/DN]
	 *    1: gain in the G channel [e-/DN]
	 *    2: gain in the B channel [e-/DN]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetDetectorGain (45)
	 */

extern double *pan_protocol_get_detector_bias(SOCKET s, unsigned long c, unsigned long *pn);
	/*
	 * pan_protocol_get_detector_bias(s,c,&n) returns the RGB bias of
	 * camera c as an array setting n to the length.
	 * If any element was invalid (e.g. the camera does not exist or does
	 * not have a detector model) then NULL is returned and n is set to 0.
	 *
	 * The returned array elements are:
	 *    0: bias in the R channel [e-]
	 *    1: bias in the G channel [e-]
	 *    2: bias in the B channel [e-]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetDetectorBias (46)
	 */

extern double *pan_protocol_get_detector_QE(SOCKET s, unsigned long c, unsigned long *pn);
	/*
	 * pan_protocol_get_detector_QE(s,c,&n) returns the RGB QE of camera c
	 * as an array setting n to the length.
	 * If any element was invalid (e.g. the camera does not exist or does
	 * not have a detector model) then NULL is returned and n is set to 0.
	 *
	 * The returned array elements are:
	 *    0: QE in the R channel
	 *    1: QE in the G channel
	 *    2: QE in the B channel
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetDetectorQE (47)
	 */

extern double *pan_protocol_get_detector_well_capacity(SOCKET s, unsigned long c, unsigned long *pn);
	/*
	 * pan_protocol_get_detector_well_capacity(s,c,&n) returns the RGB
	 * well capacity of camera c as an array setting n to the length.
	 * If any element was invalid (e.g. the camera does not exist or does
	 * not have a detector model) then NULL is returned and n is set to 0.
	 *
	 * The returned array elements are:
	 *    0: well capacity in the R channel [e-]
	 *    1: well capacity in the G channel [e-]
	 *    2: well capacity in the B channel [e-]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetDetectorWellCapacity (48)
	 */

extern double *pan_protocol_get_detector_readout_RMS(SOCKET s, unsigned long c, unsigned long *pn);
	/*
	 * pan_protocol_get_detector_readout_RMS(s,c,&n) returns the RGB
	 * readout RMS level of camera c as an array setting n to the length.
	 * If any element was invalid (e.g. the camera does not exist or does
	 * not have a detector model) then NULL is returned and n is set to 0.
	 *
	 * The returned array elements are:
	 *    0: readout RMS in the R channel [e-]
	 *    1: readout RMS in the G channel [e-]
	 *    2: readout RMS in the B channel [e-]
	 *
	 * To free the result array use free().
	 *
	 * IMPLEMENTS GetDetectorReadoutRMS (49)
	 */
...
extern void pan_protocol_set_thermal_source(SOCKET, unsigned long, unsigned long, double, unsigned char, double, unsigned char, double,  unsigned char);
	/*
	 * pan_protocol_set_thermal_source(s,oid,hid,pwr_v,pwr_m,are_v,are_m,mf_v,mf_m) 
	 * is used to update fields of thermal source hid in object oid.
	 *
	 * If pwr_m is non-zero then power is set to pwr_v.
	 * If are_m is non-zero then area is set to are_v.
	 * If mf_m  is non-zero then merge factor is set to mf_v.
	 *
	 * IMPLEMENTS SetThermalSource (300)
	 */

extern void pan_protocol_set_thermal_area_factors(SOCKET, unsigned long, double, unsigned char, double, unsigned char, double, unsigned char, double, unsigned char, double, unsigned char, double, unsigned char) ;
	/*
	 * pan_protocol_set_thermal_area_factors(s,oid,amb_v,amb_m,dir_v,dir_m,alb_v,alb_m,emi_v,emi_m,int_v,int_m,pla_v,pla_m) 
	 * is used to update the global thermal area factors of object oid.
	 *
	 * If amb_m != 0 the ambient area factor is set to amb_v.
	 * If dir_m != 0 the direct solar area factor is set to dir_v.
	 * If alb_m != 0 the indirect/albedo area factor is set to alb_v.
	 * If emi_m != 0 the planet emission area factor is set to emi_v.
	 * If int_m != 0 the internal heat factor is set to int_v.
	 * If pla_m != 0 the plate/appendage area factor is set to pla_v.
	 *
	 * IMPLEMENTS SetThermalAreaFactors (301)
	 */

extern void pan_protocol_set_emission_scale(SOCKET, double, double, double);
	/*
	 * pan_protocol_set_emission_scale(s,r,g,b) is used to set the global
	 * emission scale to (r,g,b).
	 *
	 * IMPLEMENTS SetEmissionScale (302)
	 */


extern void pan_protocol_set_irradiance(SOCKET, unsigned long, double, double, double);
	/*
	 * pan_protocol_set_irradiance(s,x,r,g,b) is used to set the irradiance
	 * of band x to (r,g,b).
	 *
	 * IMPLEMENTS SetIrradiance (303)
	 */


extern void pan_protocol_set_wavelength(SOCKET, unsigned long, double, double, double);
	/*
	 * pan_protocol_set_wavelength(s,x,r,g,b) is used to set the wavelength
	 * of band x to (r,g,b).
	 *
	 * IMPLEMENTS SetWavelength (304)
	 */

extern void pan_protocol_set_bandwidth(SOCKET, unsigned long, double, double, double);
	/*
	 * pan_protocol_set_bandwidth(s,x,r,g,b) is used to set the bandwidth
	 * of band x to (r,g,b).
	 *
	 * IMPLEMENTS SetBandwidth (305)
	 */

extern void pan_protocol_set_camera_band(SOCKET, unsigned long, unsigned long);
	/*
	 * pan_protocol_set_camera_band(s,c,b) is used to set the band
	 * of camera c to b.
	 *
	 * IMPLEMENTS SetCameraBand (306)
	 */

extern void pan_protocol_set_detector_exposure(SOCKET s, unsigned long c, double t);
	/*
	 * pan_protocol_set_detector_exposure(s,c,t) is used to set the
	 * exposure time of camera c to t seconds.
	 *
	 * IMPLEMENTS SetDetectorExposure (307)
	 */

extern void pan_protocol_set_detector_temperature(SOCKET s, unsigned long c, double t);
	/*
	 * pan_protocol_set_detector_temperature(s,c,t) is used to set the
	 * temperature of camera c to t Kelvin
	 *
	 * IMPLEMENTS SetDetectorTemperature (308)
	 */

extern void pan_protocol_set_detector_gain(SOCKET s, unsigned long c, double r, double g, double b);
	/*
	 * pan_protocol_set_detector_gain(s,c,r,g,b) is used to set the gain
	 * of camera c to (r,g,b) in units of e-/DN.
	 *
	 * IMPLEMENTS SetDetectorGain (309)
	 */

extern void pan_protocol_set_detector_bias(SOCKET s, unsigned long c, double r, double g, double b);
	/*
	 * pan_protocol_set_detector_bias(s,c,r,g,b) is used to set the bias
	 * of camera c to (r,g,b) in units of e-.
	 *
	 * IMPLEMENTS SetDetectorBias (310)
	 */

extern void pan_protocol_set_detector_QE(SOCKET s, unsigned long c, double r, double g, double b);
	/*
	 * pan_protocol_set_detector_QE(s,c,r,g,b) is used to set the QE
	 * of camera c to (r,g,b).
	 *
	 * IMPLEMENTS SetDetectorQE (311)
	 */

extern void pan_protocol_set_detector_well_capacity(SOCKET s, unsigned long c, double r, double g, double b);
	/*
	 * pan_protocol_set_detector_well_capacity(s,c,r,g,b) is used to set
	 * the well capacity of camera c to (r,g,b) in units of e-.
	 *
	 * IMPLEMENTS SetDetectorWellCapacity (312)
	 */

extern void pan_protocol_set_detector_readout_RMS(SOCKET s, unsigned long c, double r, double g, double b);
	/*
	 * pan_protocol_set_detector_readout_RMS(s,c,r,g,b) is used to set
	 * the readout RMS level of camera c to (r,g,b) in units of e-.
	 *
	 * IMPLEMENTS SetDetectorReadoutRMS (313)
	 */
 ...
 * a TX (transmission) function and an RX (reception) function. These can
 * be used if you have your own error handling and don't want to exit() on
 * an error. The individual functions above call these RX/TX functions.
 */
...
extern char *pan_net_get_thermal_sources_TX(SOCKET, unsigned long);
extern thermal_source_data *pan_net_get_thermal_sources_RX(SOCKET, unsigned long *);

extern char *pan_net_get_thermal_source_TX(SOCKET, unsigned long, unsigned long);
extern double *pan_net_get_thermal_source_RX(SOCKET, unsigned long *);

extern char *pan_net_get_thermal_area_factors_TX(SOCKET, unsigned long);
extern double *pan_net_get_thermal_area_factors_RX(SOCKET, unsigned long *);

extern char *pan_net_get_emission_scale_TX(SOCKET);
extern double *pan_net_get_emission_scale_RX(SOCKET, unsigned long *);

extern char *pan_net_get_irradiance_TX(SOCKET, unsigned long);
extern double *pan_net_get_irradiance_RX(SOCKET, unsigned long *);

extern char *pan_net_get_wavelength_TX(SOCKET, unsigned long);
extern double *pan_net_get_wavelength_RX(SOCKET, unsigned long *);

extern char *pan_net_get_bandwidth_TX(SOCKET, unsigned long);
extern double *pan_net_get_bandwidth_RX(SOCKET, unsigned long *);

extern char *pan_net_get_detector_exposure_TX(SOCKET s, unsigned long c);
extern double pan_net_get_detector_exposure_RX(SOCKET s, char *perr);

extern char *pan_net_get_detector_temperature_TX(SOCKET s, unsigned long c);
extern double pan_net_get_detector_temperature_RX(SOCKET s, char *perr);

extern char *pan_net_get_detector_gain_TX(SOCKET s, unsigned long c);
extern double *pan_net_get_detector_gain_RX(SOCKET s, unsigned long *pn);

extern char *pan_net_get_detector_bias_TX(SOCKET s, unsigned long c);
extern double *pan_net_get_detector_bias_RX(SOCKET s, unsigned long *pn);

extern char *pan_net_get_detector_QE_TX(SOCKET s, unsigned long c);
extern double *pan_net_get_detector_QE_RX(SOCKET s, unsigned long *pn);

extern char *pan_net_get_detector_well_capacity_TX(SOCKET s, unsigned long c);
extern double *pan_net_get_detector_well_capacity_RX(SOCKET s, unsigned long *pn);

extern char *pan_net_get_detector_readout_RMS_TX(SOCKET s, unsigned long c);
extern double *pan_net_get_detector_readout_RMS_RX(SOCKET s, unsigned long *pn);
...
extern char *pan_net_set_thermal_source_TX(SOCKET, unsigned long, unsigned long, double, unsigned char, double, unsigned char, double, unsigned char);
/*           pan_net_set_thermal_source_RX() is not required.  */

extern char *pan_net_set_thermal_area_factors_TX(SOCKET, unsigned long, unsigned long, double, unsigned char, double, unsigned char, double, unsigned char, double, unsigned char, double, unsigned char, double, unsigned char);
/*           pan_net_set_thermal_area_factors_RX() is not required.  */

extern char *pan_net_set_emission_scale_TX(SOCKET, double, double, double);
/*           pan_net_set_emission_scale_RX() is not required.  */

extern char *pan_net_set_irradiance_TX(SOCKET, unsigned long, double, double, double);
/*           pan_net_set_irradiance_RX() is not required.  */

extern char *pan_net_set_wavelength_TX(SOCKET, unsigned long, double, double, double);
/*           pan_net_set_wavelength_RX() is not required.  */

extern char *pan_net_set_bandwidth_TX(SOCKET, unsigned long, double, double, double);
/*           pan_net_set_bandwidth_RX() is not required.  */

extern char *pan_net_set_camera_band_TX(SOCKET, unsigned long, double, double, double);
/*           pan_net_set_camera_band_RX() is not required.  */

extern char *pan_net_set_detector_exposure_TX(SOCKET s, unsigned long c, double t);
/*           pan_net_set_detector_exposure_RX() is not required */

extern char *pan_net_set_detector_temperature_TX(SOCKET s, unsigned long c, double t);
/*           pan_net_set_detector_temperature_RX() is not required */

extern char *pan_net_set_detector_gain_TX(SOCKET s, unsigned long c, double r, double g, double b);
/*           pan_net_set_detector_gain_RX() is not required */

extern char *pan_net_set_detector_bias_TX(SOCKET s, unsigned long c, double r, double g, double b);
/*           pan_net_set_detector_bias_RX() is not required */

extern char *pan_net_set_detector_QE_TX(SOCKET s, unsigned long c, double r, double g, double b);
/*           pan_net_set_detector_QE_RX() is not required */

extern char *pan_net_set_detector_well_capacity_TX(SOCKET s, unsigned long c, double r, double g, double b);
/*           pan_net_set_detector_well_capacity_RX() is not required */

extern char *pan_net_set_detector_readout_RMS_TX(SOCKET s, unsigned long c, double r, double g, double b);
/*           pan_net_set_detector_readout_RMS_RX() is not required */
}
#endif